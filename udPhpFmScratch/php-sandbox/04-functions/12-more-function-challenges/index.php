...
echo '100&deg; Fahrenheit = ' . number_format(fahrenheitToCelsius(100), 2) . '&deg; Celsius. <br />';
echo '68&deg; Fahrenheit = ' . number_format(fahrenheitToCelsius(68), 2) . '&deg; Celsius. <br />';
...
function printNamesToUpperCase($names) {
  foreach($names as $name) echo strtoupper($name) . ' ';
}
printNamesToUpperCase(['John', 'Jack', 'Jill']);
echo '<br>';
...
function findLongestWord($sentence) {
  $words = explode(' ', $sentence); $longestWord = '';
  foreach($words as $word) { $word = trim($word);
    if(strlen($word) > strlen($longestWord)) $longestWord = $word;
  }
  return $longestWord;
}
